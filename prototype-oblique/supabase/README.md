...
All numbered schema changes through `catalog_view_loading` are already applied to Data Catalog. The sections below describe their dependencies for recovery or a fresh project; they are not a pending deployment queue. Current schema: **20 catalog tables / 483 columns**. The latest hosted migration is `20260914153224_catalog_view_loading`. [Hosted verification](../docs/review/2026-09-14-view-loading-implementation.md#hosted-rollout).
...
The Supabase app now uses `catalog.read_catalog_index(if_version text DEFAULT NULL)` for startup and `catalog.read_record(record_table text, record_id uuid, if_version text DEFAULT NULL)` per profile. Both are STABLE SECURITY INVOKER reads under the caller’s RLS. The public read-only `catalog.catalog_state` singleton advances transactionally from the existing write-serializing trigger. Index and bundle responses expose its revision as text; matching `if_version` returns a small `notModified` response. The index omits all child rows and relationship evidence. Bundles resolve inheritance and counterpart children in SQL and preserve archived owned rows for restoration. `20260914040000_catalog_view_loading.sql` was applied through the MCP on 14 September as hosted version `20260914153224`; a missing index RPC still falls back to the legacy snapshot on older projects. No role, timeout or Auth setting changed. `read_snapshot(true,false)` remains available for full catalog Excel exports, and the existing no-argument snapshot and `read_history` array contracts remain unchanged. See [view loading](../docs/review/2026-09-14-view-based-loading.md).
...
The prototype retains client-side search, sorting and pagination over the index. Server-side search and pagination remain future scaling work. Reloading revalidates current database data; there is no realtime subscription. `#/api` documents public table, index, bundle, snapshot and history reads and authenticated CRUD. Swagger supplies the public key for reads and the current app token for writes. Standard CRUD routes require the later SQL migration and `catalog-api` Edge Function; see [API activation](../docs/api.md#activation).
