-- Persistent display order for tables within their system. Existing content is preserved.
BEGIN;
SET LOCAL search_path = pg_catalog;
SET LOCAL lock_timeout = '10s';
DO $$ BEGIN
 IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema='catalog' AND table_name='data_table' AND column_name='access_options') THEN
  RAISE EXCEPTION 'Apply catalog_access_options before catalog_table_order' USING ERRCODE='55000';
 END IF;
END $$;
ALTER TABLE catalog.data_table ADD COLUMN sort_order integer NOT NULL DEFAULT 0 CHECK (sort_order >= 0);
COMMENT ON COLUMN catalog.data_table.sort_order IS 'Row order. Display order within the system, from 0 through 2147483647 (SQL integer); defaults to 0. Reordering preserves identity. Ties are permitted. Alias (DE): Zeilenreihenfolge. Canonical: DataTable.sortOrder.';
