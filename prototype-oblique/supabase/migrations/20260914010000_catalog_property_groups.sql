-- Flexible, optional group labels on business attributes and source/API fields.
BEGIN;
SET LOCAL search_path=pg_catalog;
SET LOCAL lock_timeout='10s';
ALTER TABLE catalog.business_attribute ADD COLUMN property_group catalog_private.nonempty_text;
ALTER TABLE catalog.data_field ADD COLUMN property_group catalog_private.nonempty_text;
COMMENT ON COLUMN catalog.business_attribute.property_group IS 'Group. Optional authored group label, such as a property set, technical table/structure ID or local grouping. One flexible value; no group entity, vocabulary, hierarchy or inheritance. Null means ungrouped. Alias (DE): Gruppe. Canonical: BusinessAttribute.propertyGroup.';
COMMENT ON COLUMN catalog.data_field.property_group IS 'Group. Optional authored group label, such as a property set, technical table/structure ID or local grouping. One flexible value; no group entity, vocabulary, hierarchy or inheritance. Null means ungrouped. Alias (DE): Gruppe. Canonical: DataField.propertyGroup.';

CREATE OR REPLACE FUNCTION catalog_private.edit_columns(tab text)
 RETURNS text[]
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO ''
...
SELECT CASE WHEN tab = 'service_endpoint' THEN ARRAY['url','relative_path','protocol','http_method','operation_name','environment','is_read_only','supports_bulk','authentication_methods','sort_order','is_archived']
 ...
 CASE tab
  ...
  WHEN 'business_attribute' THEN ARRAY['property_group','semantic_name','value_specification','is_identifier','code_list_id','required','system_of_record_id']
  WHEN 'data_table' THEN ARRAY['access_options','system_id','domain_id','technical_name','database_name','schema_name','sort_order']
  WHEN 'data_field' THEN ARRAY['property_group','technical_name','technical_name_kind','source_path','source_data_type','data_type_scope','is_required','is_nullable','key_roles','code_list_id','applies_to_type_names']
...
CREATE OR REPLACE FUNCTION catalog.edit_capabilities()
 ...
 SET search_path TO ''
AS $function$
  SELECT jsonb_build_object('version',1,'can_edit',catalog_private.can_edit_catalog(),'access_options',true,'api_fields',true,'property_groups',true);
$function$
;
