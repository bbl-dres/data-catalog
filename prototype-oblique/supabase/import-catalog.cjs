...
// Reconstruct the historical SQL Editor bundle from the committed migrations.
function sqlEditorBundle() {
  ...
  return '-- Paste this entire file into Supabase SQL Editor as postgres.\n-- Prerequisite: the catalog schema and member RLS scripts are already applied.\n-- One transaction; no existing catalog data is overwritten. Run once.\nBEGIN;\n'
    + body(read('20260906020000_catalog_public_read.sql')) + '\n'
    + body(read('20260906030000_catalog_import.sql')) + "\nCOMMIT;\nNOTIFY pgrst, 'reload schema';\n";
}
...
if (require.main === module) {
  if (process.argv.length === 2) generate();
  else if (process.argv.length === 3 && process.argv[2] === '--sql-editor-bundle') {
    const output = path.join(__dirname, '.temp', 'seed.sql');
    fs.mkdirSync(path.dirname(output), { recursive: true });
    fs.writeFileSync(output, sqlEditorBundle());
    console.log('Generated supabase/.temp/seed.sql (historical import only).');
  } else {
    console.error('Usage: node import-catalog.cjs [--sql-editor-bundle]');
    process.exitCode = 1;
  }
}
module.exports = { build, parseJson, seedSql, sqlEditorBundle, files };
