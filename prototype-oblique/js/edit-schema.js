...
(function (DK) {
  ...
  const governance = [field('organisationName','edit.organisationName'),field('organisationUrl','edit.organisationUrl','url'),ref('data_owner_id','detail.owner','actor'),ref('data_steward_id','detail.steward','actor'),ref('contact_actor_id','edit.contact','actor')];
  const formats = [select('valueType','fact.valueType',valueTypes),field('valueFormat','edit.valueFormat'),field('unit','edit.unit'),field('geometryType','edit.geometryType'),field('coordinateReferenceSystem','edit.coordinateReferenceSystem')];
  const specs = {
  ...
  function groups(table) {
    ...
    return [
      ['detail.facts', [...(status ? [select('status','fact.status',['draft','valid','retired'],{required:true})] : []), ...(['business_attribute','data_field'].includes(table) ? [field('property_group','fact.propertyGroup')] : []), ...(specs[table] || []),field('links','fact.moreInformation','urls')]],
