...
(function (DK) {
  ...
  diagram.layout = function (snapshot, settings, measure) {
    ...
    const entryList = settings.layout === 'list' && !diagram.usesRows(settings);
    ...
    const legend = entryList ? [] : wrap(tiles ? t('print.tilesHint') : snapshot.labels.legend, printableWidth, 7.5), footerY = height - margin - footerHeight;
    ...
    const entityFields = diagram.selectedFields(snapshot, settings, true), rowFields = entryList ? [] : diagram.selectedFields(snapshot, settings);
    const tableFields = tiles ? [] : [...(continuousList ? entityFields.map(f => ({ ...f, id: 'entity.' + f.id,
      labelText: f.id === 'name' && !entryList ? snapshot.entityLabel : rowFields.some(row => row.id === f.id) ? t('visibility.entryField', { field: f.labelText }) : f.labelText })) : []), ...rowFields]
    ...
    const minimumWidth = minima.reduce((sum, n) => sum + n, 0);
    ...
    const remaining = columnWidth - minimumWidth;
    if (remaining < 0) throw new Error(t('print.columnsTooWide'));
    const weights = tableFields.map(f => f.sizing.weight);
    const totalWeight = weights.reduce((sum, weight) => sum + weight, 0);
    const cellWidths = keys.map((key, index) => minima[index] + remaining * weights[index] / totalWeight);
    ...
    const sections = groups.map(group => {
      ...
      return { ...group, fields, summary, heading, details, headingHeight: heading.length ? Math.max(heading.length * 14 + 14, details.length * 11) + 16 : 0 };
    });
    ...
    for (const section of sections) {
      ...
      for (const entity of section.items) {
        if (continuousList) {
          const rows = (entryList ? [{ id: entity.id, display: entity.display }] : entity.rows.length ? entity.rows : [{ id: '', empty: true, display: {} }]).map(row => {
    ...
    const overview = [];
    // Reserve page references before pagination; at most one contents page is needed per section.
    const lastPageBound = pages.length + sections.length;
    const overviewHeading = { label: snapshot.groupings.find(group => group.id === settings.groupBy)?.label || t('print.group'), height: 58,
    ...
    if (entities.length && settings.groupBy !== 'none' && (settings.overview === 'yes' || settings.overview === 'auto' && groups.length >= 3)) {
      ...
      for (const section of overviewRows) {
        const lines = wrap(section.title, textWidth, 11, true), details = wrap(section.summary, textWidth, 9);
    ...
    return { width, height, margin, gap, pad, fontSize, lineHeight, bodyTop, bodyBottom, columns, keys, cellWidths, columnLabels,
      ...
      pages, pageHeadings, listHeadings, continuousList, tiles, entryList, overview, overviewHeading, entityCount: entities.length, fieldCount,
  ...
  diagram.pageSvg = function (snapshot, settings, layout, pageIndex, palette, logo) {
    ...
    const columnHeader = (x, y, w, h) => {
      ...
      layout.columnLabels.forEach((label, index) => {
        const numeric = layout.tableFields[index].type === 'number';
        write([label], x + (numeric ? layout.cellWidths[index] - 6 : 6), y + 14, 8, true, palette.text, 11, numeric ? 'end' : 'start');
        x += layout.cellWidths[index];
      });
    };
    ...
    if (layout.overview[pageIndex]) {
      ...
      for (const row of layout.overview[pageIndex]) {
        shapes.push(`<g data-contents-group="${ui.esc(row.id)}">`);
        write(row.lines, left, y + 16, 11, true, palette.text, 15);
        write(row.details, left, y + row.lines.length * 15 + 16, 9, false, palette.secondary, 12);
