...
(async () => {
  ...
  try {
    ...
    const orderChoice=page.locator('.ob-field-picker [value="sortOrder"]');
    assert.equal(await orderChoice.isChecked(),false);
    await orderChoice.check();await page.locator('[data-fields-close]').click();
    const positions=async()=>Array.from(await page.locator('.ob-detail-rows tbody [data-field="sortOrder"]').allTextContents(),Number);
    const savedPositions=await positions();
    assert.deepEqual(savedPositions,await page.evaluate(()=>DK.data.objOf('gebaeude').attributes.map(a=>a.sortOrder)));
    const byName=new Map(saved.map((name,i)=>[name,savedPositions[i]]));
    await page.locator('[data-sort-field="name"]').click();
    assert.deepEqual(await positions(),(await names()).map(name=>byName.get(name)),'Sorting never renumbers positions');
    await page.reload();await page.locator('.ob-detail-rows tbody').waitFor();
    assert(page.url().includes('sort=name'));
    await page.locator('[data-sort-field="sortOrder"]').click();
    assert.deepEqual(await names(),saved);assert.deepEqual(await positions(),savedPositions);
    await page.reload();await page.locator('.ob-detail-rows tbody').waitFor();
    assert.deepEqual(await names(),saved,'Saved-rank sorting and visibility survive reload');
    await page.locator('[data-sort-field="sortOrder"]').click();
    assert.deepEqual(await positions(),savedPositions.slice().reverse(),'Positions sort numerically in both directions');
    await page.locator('[data-sort-field="sortOrder"]').click();
    ...
    assert((await page.locator('#diagram-sheets').textContent()).includes('Zeilenreihenfolge'),'Print includes the selected rank column');
    ...
    console.log('Row-order browser: hidden archives, movement, optional saved-rank column, numeric sorting, URL/reload, print, filtering and mobile passed.');
