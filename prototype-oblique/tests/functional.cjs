  ...
  try {
    ...
    await check('handbook source links and native video work on laptop and phone', async () => {
      for (const width of [1280, 390]) {
        ...
        assert.equal(await video.locator('source').getAttribute('src'), 'assets/bbl-datenkatalog-v2.mp4');
        assert.equal(await video.getAttribute('poster'), 'assets/bbl-datenkatalog-v2-poster.jpg');
        assert.deepEqual(await video.evaluate(el => [el.videoWidth, el.videoHeight, Math.round(el.duration)]), [1920, 1080, 82]);
        ...
        assert.equal(await video.evaluate(el => el.textTracks[0].cues.length), 28);
