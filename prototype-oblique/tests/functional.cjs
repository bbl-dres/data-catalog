...
(async () => {
  ...
  try {
    ...
    await check('handbook source links and native video work on laptop and phone', async () => {
      for (const width of [1280, 390]) {
        ...
        assert.equal(await page.locator('.ob-manual-content iframe').count(), 0);
        const video = page.locator('.ob-video-player');
        await page.waitForFunction(() => document.querySelector('.ob-video-player')?.readyState >= 1);
        const rect = await video.boundingBox();
        assert(Math.abs(rect.width / rect.height - 16 / 9) < 0.01, 'Video aspect ratio');
        assert.equal(await video.evaluate(el => el.controls && el.playsInline && !el.autoplay && el.paused), true);
        assert.equal(await video.locator('source').getAttribute('src'), 'assets/bbl-datenkatalog-landscape.mp4');
        assert.equal(await video.getAttribute('poster'), 'assets/bbl-datenkatalog-landscape-poster.jpg');
        assert.deepEqual(await video.evaluate(el => [el.videoWidth, el.videoHeight, Math.round(el.duration)]), [1920, 1080, 76]);
        assert.equal(await video.locator('track[kind="subtitles"]').getAttribute('srclang'), 'de');
        await video.evaluate(el => { el.textTracks[0].mode = 'showing'; });
        await page.waitForFunction(() => document.querySelector('.ob-video-player').textTracks[0].cues?.length > 0);
        assert.equal(await video.evaluate(el => el.textTracks[0].cues.length), 22);
        await video.evaluate(async el => { el.muted = true; await el.play(); });
        await page.waitForFunction(() => document.querySelector('.ob-video-player').currentTime > 0);
        await video.evaluate(el => el.pause());
        assert.equal(await page.locator('#manual-video-transcript, .ob-video-preview').count(), 0);
        assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
      }
    });
    ...
    await check('collection search keeps typing stable and filters tiles, tables, grouping and exports together', async () => {
      ...
      assert(await page.locator('.ob-tile[data-href="#/objects/gebaeude"]').isVisible(), 'search reveals matching collapsed groups');
      assert(await input.evaluate(el => el === window.collectionInputNode && el === document.activeElement));
      const ids = await page.locator('.ob-tile').evaluateAll(els => els.map(el => el.dataset.href).sort());
      ...
      assert.equal(workbook.getWorksheet('Geschäftsobjekte').rowCount, ids.length + 2);
    ...
    await check('repeated domain branches keep their section, members, breadcrumbs and history', async () => {
      for (const kind of ['products', 'refs', 'apis']) {
        ...
        const download = page.waitForEvent('download'); await page.click('[data-export="xlsx"]');
        const workbook = await require('./excel-helpers.cjs').readWorkbook(await (await download).path());
        assert.equal(workbook.getWorksheet({ products: 'Datenprodukte', refs: 'Referenzdaten', apis: 'APIs' }[kind]).rowCount, count + 2);
    ...
    await check('domain tabs share collection layouts, filters, links and Excel scope', async () => {
      for (const width of [1440, 390, 320]) {
        ...
        assert.deepEqual(await page.locator('#collection-view-panel th .ob-table-sort').allTextContents(), ['Name', 'Beschreibung', 'Verantwortliche Organisation', 'Attribute', 'Status']);
      ...
      assert.equal(workbook.getWorksheet('Geschäftsobjekte').rowCount, 3);
      assert.equal(require('./excel-helpers.cjs').columnValues(workbook.getWorksheet('Geschäftsobjekte'), 'name')[0], 'Areal');
    ...
    await check('domain deep links, cards, tables and breadcrumbs keep the selected navigation model', async () => {
      for (const width of [1440, 390]) for (const nav of ['entity', 'container']) {
        ...
        for (const mode of ['tiles', 'table']) {
          ...
          const link = page.locator(mode === 'tiles' ? '#collection-view-panel .ob-tile-name' : '#collection-view-panel td.is-primary a').first();
    ...
    await check('Swagger keeps its mounted node, filter and expansion across chrome and viewport changes', async () => {
      ...
      await page.fill('#swagger-ui .operation-filter-input', 'Business objects');
      ...
      assert.equal(await page.inputValue('#swagger-ui .operation-filter-input'), 'Business objects');
