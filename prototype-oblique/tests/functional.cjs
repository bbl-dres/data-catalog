  ...
  try {
    ...
    await check('handbook source links and native video work on laptop and phone', async () => {
      for (const width of [1280, 390]) {
        ...
        await page.waitForFunction(() => document.querySelector('.ob-video-player')?.readyState >= 2);
        ...
        assert.equal(await video.locator('source').getAttribute('src'), 'assets/bbl-datenkatalog-70s-clean.mp4');
