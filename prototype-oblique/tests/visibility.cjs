...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-diagram-layout]').count(), 3);
    ...
    assert.deepEqual(await page.evaluate(() => window.printTest.layout.keys), ['entity.name', 'entity.version', 'entity.status'], 'Collection List uses exactly the visible web fields');
    assert.equal(await page.evaluate(() => window.printTest.layout.pages.flat().flatMap(card => card.rows).length), 9, 'One row per catalog entry');
    ...
    assert.deepEqual(await page.evaluate(() => window.printTest.layout.keys.map(key => key.replace('entity.', ''))), inheritedFields);
    await page.locator('.ob-export-header [data-diagram-action="close"]').click();
    await visit('#/objects/gebaeude?tab=rows');
    await page.locator('[data-tab="rows"]').click();
    await open();
    assert(await page.evaluate(() => window.printTest.settings.listRows), 'Attribute tables retain detailed print rows');
    ...
    assert(result.headerCounts.every(n => n === 1));
    ...
    const printCases = await page.evaluate(async () => {
      ...
      const svg = new DOMParser().parseFromString(DK.diagram.pageSvg(snapshot, settings, layout, 0, DK.pdf.palette(), ''), 'image/svg+xml');
