...
(async () => {
  ...
  try {
    ...
    const crudRequest=(path,method,headers={})=>request('https://catalog.example/functions/v1/catalog-api/'+path,method,headers);
    const id='8c965b13-447c-4a66-bb17-7b9e0b791cb0',jwt='test.payload.signature';
    const write=prepare(crudRequest('domain/'+id,'PATCH',{'If-Match':'"1"','Idempotency-Key':id}),spec,target,jwt);
    assert.equal(write.headers.authorization,'Bearer '+jwt);assert.equal(write.headers['content-type'],'application/json');assert(!write.headers['content-profile']);
    assert.throws(()=>prepare(crudRequest('domain','POST'),spec,target),/current app access token/);
    for(const path of ['change_event','domain/not-a-uuid','domain/'+id+'/extra','../domain/'+id])assert.throws(()=>prepare(crudRequest(path,'PATCH'),spec,target,jwt),/Only documented/);
    ...
    console.log(`PASS: generated CRUD contract, SQL columns/keys, 84 read/write operations, credential/destination guards and ${evidence.source}. JSON Schema evidence: ${folder}`);
