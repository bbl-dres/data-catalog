...
(async () => {
  ...
  try {
    ...
    await page.locator('#swagger-ui .scheme-container .authorize').click();
    assert.match(await page.locator('.dialog-ux').innerText(), /BearerAuth/);
    assert.doesNotMatch(await page.locator('.dialog-ux').innerText(), /PublishableKey/);
