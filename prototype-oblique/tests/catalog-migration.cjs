...
const { build, parseJson, sqlEditorBundle, files } = require('../supabase/import-catalog.cjs');
...
(async () => {
  ...
  try {
    ...
    try {
      ...
      await assert.rejects(occupied.exec(sqlEditorBundle()), /requires an empty catalog/); checks++;
