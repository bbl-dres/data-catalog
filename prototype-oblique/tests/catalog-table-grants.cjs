...
(async()=>{
 ...
 try{
  ...
  assert.equal(check.columns,478);assert.equal(check.read_only,true);
  console.log('PASS: reproduced hosted write-grant drift; repair is repeatable, preserves all rows and SELECT, and all current incremental migrations apply with 478 columns and no direct API-role writes.');
