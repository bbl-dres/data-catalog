...
`catalog-table-grants.cjs` reproduces the direct table write grants found during hosted activation, applies the scoped repair twice, verifies preserved content and SELECT access, and then applies all current incremental migrations with 478 resulting columns and no direct API-role writes. It uses the same PGlite setup as the SQL suites.
...
| `model-aliases.cjs` | Canonical Markdown against all 478 actual SQL/API column aliases, nested JSON labels, generated EN/DE copy, form/column definitions, print snapshots, XLSX headers and distinct definition/service versions; requires PGlite |
| `table-order.cjs` | DataTable rank migration, guarded browser/REST edits, validation, retry/revision/no-op history, denied direct writes, default sorting, numeric editor/export and matching Attribute/Felder column/dropdown order; requires PGlite |
| `access-options.cjs` | Owned access lists on all three record types: real SQL browser/REST validation, identity retention, revision/retry/history, safe projections, XLSX write/read and print/API contracts; requires PGlite |
| `access-options-browser.cjs` | Expandable access descriptions, keyboard/native print, all three editors, validation/order/archive/restore, old-schema compatibility and 320–1440 px layouts in four languages; requires Playwright and PGlite. Uses isolated SQL and intercepts every hosted request; evidence in the temporary `oblique-access-options` folder |
| `model-alias-browser.cjs` | Visible DE/EN aliases in mobile/desktop tables and profiles, code-list source authority, and actual PDF/XLSX downloads; requires Playwright. Set `DIAGRAM_SUPABASE=1` and `PGLITE_MODULE` for the SQL fixture; validate PDFs with `python prototype-oblique/tests/diagram-pdf.py canonical-aliases-de canonical-aliases-en` |
...
| `system-of-record.cjs` / `system-of-record-browser.cjs` | System UUID FKs, browser/REST audit and validation, no text backfill, object default/attribute override and clearing, renamed/archived system labels, linked profile and mobile dropdown; requires PGlite, plus Playwright for the browser suite |
| `row-order.cjs` / `row-order-browser.cjs` | Consistent ranks across five child types, browser/REST append, archive positions, retries, invalid ranks, Excel values and languages, visible-row movement, optional rank column, numeric sorting and print inheritance; PGlite and Playwright |
...
| `functional.cjs` | Handbook legacy links/navigation, inline official sources and responsive native video playback; hero/header search examples, domain/type filters, one result table, global pagination/sorting, mock answers and URLs; collection filtering, IME, export and history; shared domain tabs, legacy links, scoped export; collapsible overview sections, default states, keyboard/touch, tab/re-render persistence and print; focus, menus, API lifecycle/races/retry and load failures |
...
| `list-search.cjs` | Shared collection/detail/history search: complete row coverage, technical names/descriptions/actors, 150-event history paging and sorting, history visibility/reset/persistence, IME/focus, empty states, independent URL restoration, responsive controls and full entity export |
...
| `sap.cjs` | Curated SAP scope, source inventories, alphabetical tree, retired entries, architectural types, field search, documentation/comments, responsive layouts, source-type preservation and Excel review columns |
| `gis.cjs` | GIS workbook scope, Bodenabdeckung type Gebäude, duplicate field identities, source statuses, pagination/search, mobile layouts and system export scope and all 275 fields through table exports |
| `av.cjs` | AV model/service navigation, field search, value-list links, hidden source facts, stable empty metadata, mobile layouts and system export scope and all 49 fields through table exports |
