...
(async () => {
  ...
  try {
    ...
    for (const width of [1920, 1440, 390, 320]) {
      ...
      const fonts = await page.locator('#panel-rows tbody tr').first().locator('.ob-cell-value').evaluateAll(els => els.slice(0, 2).map(el => getComputedStyle(el).fontFamily));
      assert.equal(fonts[0], fonts[1], 'field names use the same font as descriptions');
      ...
      assert.equal(await page.locator('.ob-system-facts dl').isVisible(), true);
      assert(await page.locator('.ob-system-facts').evaluate(el=>el===el.parentElement.lastElementChild),'System is last');
      ...
      assert(page.url().includes(link));
      await page.reload(); await page.locator('.ob-core-facts').waitFor();
      assert.equal(await page.locator('#tab-overview').getAttribute('aria-selected'), 'true');
      console.log(`PASS: field navigation, labels, fonts, breadcrumbs, reload and layout at ${width}px`);
    }
    ...
    await page.click('[data-menu="actions"]');
    assert.equal(await page.locator('.ob-actions-menu [data-export*="pdf"]').count(), 0);
    ...
    const language = async lang => {
      ...
      await page.locator(`#language-host [data-lang="${lang}"]`).click();
      await settle(page);
    };
    await language('fr');
