...
(async()=>{const server=createServer({catalogProvider:'supabase'});let browser;try{
 ...
 await page.route('https://zicluerzbevodlmtbxow.supabase.co/**',route=>{
  assert(['/rest/v1/rpc/read_snapshot','/rest/v1/rpc/read_catalog_index','/rest/v1/rpc/read_record','/rest/v1/rpc/read_history'].includes(new URL(route.request().url()).pathname) && route.request().method()==='POST','Only public read RPCs allowed');
 ...
 await page.waitForFunction(()=>!DK.data.historyState('tables','t-sap-building').loading);
 ...
 assert.equal(await page.evaluate(async()=>{await DK.data.loadRecord('apis','api-sap-building');return DK.data.get('apis','api-sap-building').fields.length;}),378);
 await visit('');
