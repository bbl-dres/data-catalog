...
- Public catalog browsing from Supabase. Self-hosted Noto Sans and pinned Swagger UI, ExcelJS, jsPDF and svg2pdf.js, with no build step. Export libraries load only when needed.
- Supabase email/password login for administrator-created accounts, persistent sessions, password changes and sign-out in DE/FR/IT/EN. Password recovery is handled by the administrator; no mail provider is required. See [login setup](supabase/README.md#email-and-password-login).
- [Edit mode](docs/edit-mode-implementation.md) for every signed-in user: create and edit profiles, translations and owned rows, save/discard drafts, reorder and archive/restore rows. Saves validate revisions and record history. Relationships remain read-only. Requires the [editing migration](supabase/README.md#enable-editing).

## Run locally

Follow the [Supabase setup](supabase/README.md#apply-to-the-existing-project) first; an already initialized project does not need seeding again. Then serve the vanilla JavaScript application over HTTP from the repository root:
...
Project code is covered by the [MIT License](../LICENSE). Oblique styles are MIT (Swiss Confederation, FOITT), Noto Sans is under the SIL Open Font License 1.1, Swagger UI is Apache 2.0, and [ExcelJS](vendor/exceljs/LICENSE), [jsPDF](vendor/jspdf/LICENSE), [svg2pdf.js](vendor/svg2pdf.js/LICENSE) and [Supabase JS](vendor/supabase/LICENSE) are MIT. The Swiss federal logo is protected by the Coat of Arms Protection Act and may only be used by federal bodies.
