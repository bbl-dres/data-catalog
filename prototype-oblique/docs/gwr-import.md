...
Source file SHA-256 hashes, capture date, per-table field coverage, source choices, excluded old codes and summary counts are in [gwr-import-report.json](gwr-import-report.json). Original MHTML and Excel files remain outside the repository.
...
```powershell
python prototype-oblique/docs/import-gwr.py --catalog "C:\Users\david\Downloads\Merkmalskatalog.mhtml" --codes "C:\Users\david\Downloads\gwr codes.xlsx"
```

[import-gwr.py](import-gwr.py) validates the expected 5.0 field coverage and duplicate codes before writing. Repeated runs with these inputs upsert the same IDs and import-history entries. `--output` and `--report` allow generating a separate review copy. The older `generate-data.py` produces fictional baseline data; rerun this import after regenerating that baseline.
