...
Total: **275 field rows — 131 LIVE, 143 DEV, one unspecified**. All catalog records remain **Entwurf** under the existing policy. The source's LIVE/DEV value is retained separately in data and Excel; it is hidden from field profiles at the user's request. Missing source status stays unspecified.
...
```powershell
python prototype-oblique/scripts/import-gis-immo.py `
