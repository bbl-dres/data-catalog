...
**Print** beside **Export** opens the output-only PDF workspace. Collections inherit their current Tiles/Table layout, fields, grouping, search scope and entry order; Table becomes print List with one row per entry. Domain/system entry points select their business objects/tables. Attribute/field detail tabs open a detailed List, including all documented child rows independent of field pagination or filtering. Grid remains available for entries with children. APIs include documented endpoints and operation details; products include attributes, business-object members, source tables and serving APIs in detailed layouts. The Export dropdown has no PDF options.
...
| Paper/orientation | A4–A0, default A3. Portrait is listed before landscape. Tiles/Grid start landscape; List starts portrait. Explicit orientation choices are retained. Physical text size is fixed; there is no print-scale control. |
...
| Grid | Cards show selected entry facts and child columns. Their width adapts to the selected columns; cards flow down columns. Each group starts a new page and repeats its band on continuation pages. |
| List | One continuous full-width table per content page. Collection entry points use the web columns and one row per entry. Detail entry points retain child rows with selected parent metadata as ordinary columns. No entity separator bands. Headers repeat per page; chosen domain/system groups remain. Inherited and explicit layout choices survive scope changes. |
| View | Available in every layout and synchronized with the main views. One compact list merges shared entry/child properties in detailed layouts; Tiles and collection List show entry choices only. Names stay selected, as does code for code values. Checkboxes and Reset update immediately. Missing values remain dashes. |
| Single entry | Grid uses the full page width, with selected entry facts on the first fragment and selected columns for every child row. List keeps its flat table structure. Entries without child rows remain visible: List emits one entry row with empty child cells and a zero field count. |
| Table of contents | Section names with a smaller count line below and right-aligned page ranges under a Page header. Automatic from three groups, or explicitly enabled/disabled. Long names wrap and contents can span pages; page references include all contents pages. Ownership is not appended to counts. |
...
Contents, section bands and document summaries use the same counts: entries in every layout, plus attributes, fields, values or endpoints when child rows are printed. Product child rows combine attributes and component references, so their count remains labelled Rows. Responsibility is shown through the selected entry metadata, or as a section title when explicitly grouping by responsibility.

Dropdown ordering stays stable across selections. Paper sizes run from A4 to A0 (small to large); preview lists fit modes before ascending zoom percentages. Grouping follows the main app (None, then entity context and status). Document status follows its lifecycle, classification increases in sensitivity, and the contents option lists Automatic, Yes, No. Filter names use the document language's alphabetical order. A selected default does not move an option to the top. The View checklist follows the shared attribute order, not alphabetical order.
