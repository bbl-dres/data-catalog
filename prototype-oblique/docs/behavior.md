...
The introduction plays `assets/bbl-datenkatalog-70s-clean.mp4` in a responsive 16:9 HTML video element with the browser's native controls, inline playback and metadata preload. Playback starts only on request. The local media source is permitted by CSP; no external player is loaded. The earlier YouTube-style mock and planned narration outline have been removed from the page.
