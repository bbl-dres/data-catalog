...
Preserve translations, comments, source metadata and source positions even when the profile does not display those facts. Write catalog strings as literal text, including formula-like content and leading-zero codes. Keep numeric values typed separately. Long text uses continuation references rather than silent truncation; worksheet names and collisions follow Excel constraints, including the reserved name "History" (the English history sheet becomes "History (2)"). Projection internals (the SQL record and relationship index behind each entry) are not metadata and never appear in the metadata sheet. Tests read downloaded workbooks back to check values, scope and completeness.
