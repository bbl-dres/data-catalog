...
The introduction plays `assets/bbl-datenkatalog-v2.mp4` (82 seconds, 1920 × 1080) in a responsive 16:9 HTML video element with the browser's native controls, inline playback, metadata preload and a local poster. German WebVTT subtitles are optional through the native player. Playback starts only on request. The local media source is permitted by CSP; no external player is loaded. The [Version 2 production source and Shotcut timeline](video/2026-09-15-version-2/README.md) combine the user's earlier Corinna recordings, a continuous building diagram and actual prototype captures demonstrating the Baujahr definition. [Version 1](video/2026-09-15-landscape/README.md), its export, production files and all earlier MP4s remain unchanged and available for comparison.

The text describes public catalog browsing and uses Innovator as the architecture modelling tool. It does not promise daily synchronization, eIAM login, automatic external publication or an implemented DCAT export. The model chapter separates local catalog entities from standards and lists actual core field names instead of presenting unverified vocabulary mappings as an exchange contract.

## Login and account

The header's **Anmelden / Sign in** button opens an accessible modal for an existing email/password account. Successful login closes the dialog and replaces the button with **Konto / Account**, preserving the catalog route. The account dialog shows the signed-in email, password change and sign-out. The SDK persists and refreshes sessions and synchronizes sign-in/sign-out across tabs. Signing out applies to this browser session; public catalog browsing continues.

The account dialog also provides Show/Hide and Copy controls for the current API access token, its expiry and a manual-copy fallback when clipboard access is unavailable. Tokens stay hidden until requested and are removed from the dialog on close, account change or logout. No refresh token is shown. External API clients use the token until expiry; Swagger can use the current app session automatically. See [CRUD API access](api.md).

**Passwort vergessen? / Forgot password?** directs users to their account administrator in the current configuration; it sends no email. Optional `authRecovery: 'email'` enables PKCE reset links after SMTP is configured. Open those links in the requesting browser. Requests disable duplicate submission, failures remain visible, passwords are cleared after requests or closing, and Escape returns focus to the header. The dialog supports all four UI languages and scrolls on short screens. There is no self-registration; see [Supabase login setup](../supabase/README.md#email-and-password-login).

An invitation sent from the Supabase dashboard uses its standard email link. On return, the app removes the link's session tokens from the URL, verifies the user with Supabase and opens the new-password form. Dashboard recovery links also open this form; confirmation and magic links restore the account. Incomplete or rejected callbacks show the invalid-link message. Email validation and sender-authorization errors have distinct messages, so delivery configuration failures are not reported only as connection failures.

## Navigation and collections

Profile overview section headings toggle their content with a chevron, using click, Enter or Space. **System**, **Kerndaten**, **Informationsschutz**, **Verantwortlich** and **Bereitstellungsformen** start expanded. System appears last, after Bereitstellungsformen where present. Each section is independent. Choices survive re-renders and tab changes within the current entry; visiting another entry or reloading restores the defaults. Collapsed sections remain included in print and exports. Edit forms keep their fields expanded.

Signed-in users can open **Edit** on a profile or **Create** on the seven collection pages after [editing activation](../supabase/README.md#enable-editing). The [edit workspace](edit-mode-implementation.md) retains one in-memory draft across its tabs, marks changed fields, validates saves and confirms discard/navigation. Owned attributes, fields, code values, product attributes and endpoints support add/edit/order/archive/restore. Relationships remain read-only. Public browsing and exported data use the last saved snapshot. Comments appear in the **System** section of profiles and edit forms, including expanded child-row details.
...
Detail overviews always use a single column, with **Responsibility** first, followed by **Key facts** and **Protection and privacy**. **System** is the final section, below any access options. Classification and personal-data handling belong to Protection and privacy. System shows identifiers, version, creation/modification dates and the existing source/synchronization metadata. Each section can be collapsed using its heading; all sections start expanded. Unknown values display dashes. System also contains Kommentar. Native print reveals the sections. The same section order applies at every viewport and sidebar width. This grouping does not change the catalog records or export contents.
...
Names are mandatory; code values also retain their code. Attribute and field tables default to saved sortOrder ascending even when Zeilenreihenfolge is hidden. Explicit column/URL sorts override the default and survive reload. Web and print use the same compact choices for each entity type: descriptions, status, ownership, domain/system context, versions, counts and essential type-specific facts. Fields and business attributes show their database lifecycle status as a pill in the last column by default. Users can hide it in Ansicht and restore it with Reset; other selected columns always precede Status. Existing v1 browser preferences gain the previously unavailable row status once; subsequent hide choices and explicit URL field selections are respected. Fields and attributes also offer data type, keys, required/nullability and value lists where applicable. Owned rows also offer **Zeilenreihenfolge / Row order**, hidden by default, showing the saved rank and supporting column sorting. Audit dates, internal IDs, comments, information links and other detailed metadata remain outside the picker, available through detail pages and the API. Empty selected values remain `—`, and zero/false remain meaningful values. Names retain alias/technical-name formatting. Tiles show descriptions, labeled facts and count/status footers; tables show the same selection as columns. Labels remain translated in DE/FR/IT/EN.

Checkbox changes and Reset take effect immediately, including the toolbar count. The menu stays open and retains focus and scroll position; its trigger closes it again by pointer or keyboard. Close, Escape and outside dismissal keep the current selection; there is no Apply or Cancel step. Preferences persist in this browser, and retired choices are ignored when reading older preferences. Hiding the active sort field restores name ordering. Search still matches hidden metadata, and Excel exports retain their fixed review columns regardless of visibility.
...
Data-table, data-product and API profiles show **Bereitstellungsformen** beneath the overview facts. The full-width section heading uses the same chevron and disclosure button as the other overview sections and starts expanded. Click, Enter or Space collapses the entire section. Each entry expands independently using the shared heading button and chevron; entries start collapsed. Collapsing the section preserves child choices. Print reveals both levels. Details describe format, status, access/download links, access instructions, terms and comments. Missing values remain dashes. On narrow workspaces, labels sit above their values so long labels and URLs remain readable. Empty lists show a short message.
...
Rank and sort the entire matching set before slicing the page. The top controls show the entry range and sort selector, without navigation arrows; on phone-width columns the sort label sits above a full-width select, like the card-sort toolbar. The bottom pager contains page navigation and page size, without a duplicate range. No matches hides the pager; a single page disables navigation.
...
Circular groups surround a central entity with undirected connections: these are associations, not lineage. Layout bounds include circles, captions and paging controls. Groups display at most six entries per page, or three below 640 px of canvas width. Paging keeps bubble size bounded; a narrow canvas arranges groups vertically. Nodes are 96 px wide and show up to two label lines before an ellipsis; titles and the selection row still carry the full name.

The inline diagram grows with its status row; its minimum is the remaining viewport height (a 380 px floor on desktop, the visible height below the sticky header on phones and tablets), capped once the whole layout fits at 100 %. Controls provide bounded zoom (15–200%), fit/reset, 100%, pan/select modes, directional movement and fullscreen; the toolbar wraps in groups on narrow shells. Auto-fit never goes below 75 % (100 % on touch screens): when the floor binds the view starts at the first group, centred on the hub, and the user pans. The open-entry link in the selection row shares the 32/44 px control height, and on phones the entry text takes the row with the open and close actions below it. Clicking a node selects it and exposes an explicit open-entry link; dragging must not activate a node. Keyboard arrows pan, `+`/`-` zoom, `0` resets, and focused offscreen entries are brought into view.
...
The API view renders the generated public-read and authenticated CRUD contract. Collection GETs support PostgREST filters, projection and pagination; the snapshot POST remains read-only. Single-record reads and writes use the catalog-api Edge Function. Swagger uses the signed-in app user's token for writes or an explicitly authorized Bearer token. POST creates, PATCH changes supplied properties and DELETE archives. Every write needs an Idempotency-Key; PATCH/DELETE additionally need the record's quoted revision in If-Match. History is read-only. Swagger retains its filter, expanded operations and inputs across application chrome updates and viewport changes; leaving the route resets the reference. Search and exports remain browser features. See the [API guide](api.md) for activation and examples.
...
Sheets have stable technical keys in row 1 and translated labels in row 2, filters on row 2, frozen headers and identifying columns, fixed widths and a feedback column. Stored ranks are exported as `sortOrder`, never inferred from row numbers. Technical UUID columns are initially hidden but remain available. Names and documented responsibility context stay visible. Dates retain their actual precision, unknown values stay blank, and strings/codes remain literal text. Oversized text has a lossless continuation sheet when necessary.

Exports capture an immutable plan before the local ExcelJS writer loads. Navigation and language changes cannot alter a pending export. Duplicate actions are guarded; failed loads can be retried. Exports use the last saved snapshot and do not import workbook feedback.

## Attribute meanings and source tables

Business-attribute relationships show their explicitly mapped source fields and deduplicated data tables. Source fields and tables link back to the represented business attributes. Only persisted `represents` assertions in candidate or confirmed state appear; rejected/archived assertions and retired endpoints are excluded. The list context, diagram tooltip and selection details distinguish **Zu prüfen** from **Bestätigt**. Similar labels do not establish equivalence. Three initial Baujahr mappings connect GIS IMMO, RE-FX and GWR as candidates. The diagram counts linked context entries (parent, tables and fields), not distinct database assertions.

## Comment policy

Keep comments short and useful for stewardship. Empty is acceptable. Definitions belong in descriptions; source evidence stays in documentation links and review records. Preserve existing property-set and key-role markers until their structured replacements are implemented.
