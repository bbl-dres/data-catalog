...
The introduction plays `assets/bbl-datenkatalog-landscape.mp4` (76 seconds, 1920 × 1080) in a responsive 16:9 HTML video element with the browser's native controls, inline playback, metadata preload and a local poster. German WebVTT subtitles are optional through the native player. Playback starts only on request. The local media source is permitted by CSP; no external player is loaded. The [production source and Shotcut timeline](video/2026-09-15-landscape/README.md) adapt the supplied portrait NotebookLM explanation with new landscape diagrams and its original audio. Both earlier MP4s remain available in `assets/`.
