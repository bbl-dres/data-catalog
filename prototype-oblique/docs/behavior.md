...
The introduction includes a local 16:9 explainer mockup with a planned 1:50 duration. Its poster was supplied by the user from `service-portal/assets/images/customer-portal-tutorial-placeholder.jpg` and copied to `assets/manual-explainer-placeholder.jpg`. The overlay follows the supplied channel reference: Swiss emblem in a white circle, video title and channel name, rounded central play symbol and a white “Watch on YouTube” badge. The emblem reuses the local SVG; all other overlays are HTML/CSS. This is not a YouTube iframe and it makes no requests to a video provider. The caption identifies it as a preview; activating the poster opens the six-part narration outline and focuses its disclosure. The badge is decorative until a real video destination exists. The separate disclosure also works without the poster action. Replace the mock with an actual player and captions when the video exists.
