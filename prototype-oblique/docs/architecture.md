...
No build step and no framework. One HTML page, shared stylesheets, application JavaScript modules, a public Supabase catalog and local UI configuration. Pinned self-hosted Swagger UI, ExcelJS and PDF libraries load only when needed. The self-hosted Supabase SDK manages optional email/password sessions. See [database setup](../supabase/README.md) for the applied SQL Editor import, login settings and hosted verification.
...
│   ├── icons.css         Generated by scripts/build-icons.py: every icon as an inline SVG mask
...
│   ├── auth.css          Responsive account, login and password-reset dialog
│   ├── editor.css        Profile drafts, child-row controls and edit validation
...
│   ├── catalog-config.js Public Supabase connection settings; explicit JSON mode for offline fixtures (the first script)
│   ├── resources.js      DK.resources – request deadlines, shared connection checks and retryable scripts/styles
│   ├── boot.js           DK.boot   – starts UI configuration and a conditional catalog-index read
│   ├── catalog.js        DK.catalog – index/bundle transport, shared projection and full-snapshot exports
│   ├── auth.js           DK.auth   – Supabase sessions and account dialog; public browsing stays independent
│   ├── edit-schema.js    DK.editSchema – curated editable fields and multilingual draft transformations
│   ├── editor.js         DK.editor – profile drafts, validation, navigation guards and atomic saves
...
│   ├── api.js            DK.api    – lazy Swagger mounting, public reads, authenticated writes and retry
...
│   ├── icons/*.svg       25 Oblique icons and 3 local diagram controls, the sources of css/icons.css
...
├── vendor/supabase/      Supabase 2.116.0 browser SDK and license/integrity notice
...
├── scripts/             Scoped Python import and source-check tools, and the icon stylesheet generator
└── docs/                Maintained guides, import instructions and source evidence
```

The application scripts are IIFEs on window.DK. catalog-config.js creates the namespace, followed by resources.js and boot.js. These scripts start the four UI files and a conditional read_catalog_index request while later modules arrive. DK.boot.take() hands out each parsed promise once; failures remain available to the normal load-error display. Requests have a 20-second deadline covering body parsing.

`resources.js` owns script/style deduplication, load deadlines, readiness checks and retry cleanup. Swagger waits for both CSS and JS. PDF loads its two dependent libraries in order; Excel loads its writer on demand. Auth loads the Supabase SDK asynchronously in Supabase mode, allowing public rendering while sessions are restored. These mechanics do not add a build step or remote runtime dependency. The configuration, resource and boot scripts remain ordered body scripts; `app.js` initializes last.

Styles load in order: tokens, generated icon masks, reusable components, application layouts/variants, relationship diagram, export workspace. `css/icons.css` inlines every `assets/icons/*.svg` as a `data:` URI custom property (`--ob-icon-<name>`) and the matching `.ob-icon-<name>` class, so a rendered page issues no icon requests; regenerate it with `python3 prototype-oblique/scripts/build-icons.py` after changing an icon. The base reset sets `box-sizing: border-box` on every element directly rather than by inheritance, so controls inside `<details>` measure like their siblings. `.ob-input` and `.ob-select` share native field styling; `.ob-icon-button` supplies quiet actions in inputs and notices; `.ob-card` supplies the filled surface, safe wrapping and interaction states for KPI cards and tiles. Layout classes retain their distinct padding, grids and content limits. See [the shared component guide](design-system.md#component-ownership) for ownership and regression checks.
...
| `resources.js` | Shared request deadlines, browser asset loading and catalog connection validation | Own application state or retain authentication tokens |
...
| `api.js` | Swagger mount ownership, documented read/CRUD request guards and just-in-time access tokens | Grant database access, bypass SQL authorization or retain login tokens |
...
Some updates are local: search typing refreshes suggestions, help/language menus refresh their hosts, the page's export and grouping menus (`#actions-menu-host`, `#group-menu-host`) swap only their host element, expanding or collapsing a tree branch re-renders `#sidebar-tree` (and the rail flyout's tree), and graph panning changes a transform. Any other control closes an open menu before it acts. A shared `ResizeObserver` distributes each table's available width above its column minima and sets the table's minimum width, so a narrower container scrolls it sideways; a second read/write pass keeps the scroll affordances current (the `is-scrollable` region, the hidden-side classes of `.ob-table-scroll`, the hint, and the scroller's `tabindex`/`role="region"`), and one capturing `scroll` listener updates them while scrolling. Every region is measured first and written afterwards, and a region already adapted to the same width and font size is skipped, so a page with many tables lays out once rather than once per table. Root custom properties (`--ob-sidebar-width`, the visual-viewport tokens) are written only when their value changes, because each write invalidates the whole document's styles; the sidebar and search code cache the token values they read.

Catalog projection indexes relationships, quality assignments and endpoints once per snapshot. Detail rows, print and Excel share direct child enrichment, avoiding another parent-list scan for every child. The editor caches patches per draft row and refreshes them only when that row changes; opening is single-flight and capability responses are checked against their latest generation. Localized SQL values resolve directly from their language columns; language changes still take effect without reloading. Table sorting reads each row's sort value once, and profile tab counts use child counts without generating hidden row markup. All locale comparisons use `ui.collator()`, a cached `Intl.Collator` per language and option set (constructing one per comparison costs about 25× a reused compare). Search scores are reused within each ranking operation, with no cache surviving a query or language change; the empty-input examples are reused while the scope, language and catalog are unchanged.

PDF text widths use a 5,000-entry cache owned by the workspace's font measurer. The key includes text, size and weight; old entries are evicted, and closing releases the measurer. Text wrapping measures each word and the space once and accumulates line widths (the measurer sums glyph advances, so the sums are exact), and only splits oversized words character by character. The print workspace captures a language × kind catalog on first access rather than all twenty on opening; the translated dictionary is shared per language. Preview scrolling keeps nearby SVG pages mounted and updates accessible page feedback only when the page changes. See the [performance review](review/2026-09-06-performance-review.md) and the [code review](review/2026-09-07-code-review.md) for measurements and limits.
...
`supabase/generate-openapi.cjs` generates `data/swagger.json` from SQL migrations and the frozen CRUD property inventory. Public PostgREST collection/snapshot reads retain schema/public-key headers and omit bearer credentials. The documented single-record GET/POST/PATCH/DELETE operations use `supabase/functions/catalog-api/index.ts`, which forwards verified user tokens to the audited `api_write()` SQL command. Swagger validates the operation's destination before obtaining the current app token, and never persists automatic authorization. Offline JSON tests disable live requests. No hosted schema discovery or administrator key is involved. See the [API guide](api.md) for activation, regeneration, schema limits and verification.
...
Field profiles reuse the attribute-page composition and have Übersicht, Beziehungen and Verlauf Datentabelle. `data.field()` derives the profile from the projected field and parent context; `router.entityHref('fields', '<tableId>/<fieldId>')` preserves its address. Every field name and row in Felder links to that profile at every width; code-list links remain separate destinations. Breadcrumbs and the Datentabelle fact return to the parent's Felder tab. Both tree models highlight the parent table. SQL names project into `technicalName` and a localized `labels` map; the profile shows both in Kerndaten. Curated documentation links appear under Weitere Informationen. Raw source payloads stay in the frozen import evidence.

The Supabase provider starts with read_catalog_index: full parent records, structural relationships without evidence, endpoints, quality rules and visible child counts. Lists, search, navigation and home use this index. data.recordState(kind,id) loads one read_record bundle on profile navigation, hover or focus; child deep links share their owner. Bundles include archived owned rows, inherited attributes and minimal relation counterparts. A counterpart is never treated as a complete owner. Every index/bundle has an exact transactional catalogVersion; mismatched reads refresh the index and retry, and old responses cannot republish after a reset. After saving, the editor reloads the index and edited bundle, discarding other in-memory owners. The Cache API retains at most 64 public responses partitioned by project and owner; every reuse is revalidated through if_version, with ordinary network reads when storage is unavailable. Full snapshots and offline JSON keep the existing projection path. read_history retains its array contract and loads per owner. Whole-catalog Excel alone requests read_snapshot(true,false), builds a temporary export projection and preserves the live store. PDF row layouts load their scope with at most four concurrent owner requests; a catalog revision change requires reopening the preview. Legacy RPC shapes and the public SQL timeout remain unchanged. Search, sorting and pagination stay in memory; server paging remains deferred.

Before publishing a snapshot, the loader checks shapes, identities, embedded lists and duplicate IDs. Invalid input leaves a previously loaded snapshot intact and never silently switches to the archived JSON. The [core tests](../tests/core.test.cjs) and [migration/browser checks](../supabase/README.md#validation) cover these boundaries. HTML data is escaped; config/data links additionally use `ui.safeHref()` through `ui.link()`. Its label HTML must already be escaped. Excel writes literal text cells, including formula-like strings, using fixed review columns. Complete canonical SQL records and relationship verification details remain available through the API. Source-only payloads excluded from the target remain in the frozen import files and source captures.
...
Collection rows and scoped Excel plans share presentation sorting; Excel columns follow the fixed [review export contract](excel-export.md), independently of visible web columns. Attribute labels in EN/DE are generated from the canonical Markdown by `scripts/sync-model-aliases.cjs`; forms, profiles, print and Excel use these shared translation keys. The [alias review](review/2026-09-13-model-alias-review.md) records the mapping and regeneration procedure. `excel.plan(route, ctx, baseUrl, { scope })` supports `selection` (default) and `catalog`. Catalog scope enumerates all seven entity kinds directly, orders each by display name and bypasses view filters and detail sorting; both scopes share worksheet construction and snapshot capture. Detail tables and search use `ui.pageState()`, `ui.pageParams()` and the options-based `ui.pager()`; page-size defaults are serialized from the resolved state. `data.fieldSourceFacts()` translates imported source headings into English property names for profile consumers, while preserving the original metadata.
...
The `change` listener also handles page sizes and search sorting. Menus support keyboard entry, arrows, Home/End, initial-letter navigation, Tab exit and Escape; drawer focus containment uses the active control after an inner menu closes.
...
- **Backend evolution**: extend the normalized schema, snapshot contract and adapter together. The [edit API](edit-mode-implementation.md) is implemented separately from public reads; server-side search remains future work. Do not reintroduce JSON as a second live writer.
...
Compound child keys are built and split through `data.childId()` / `data.splitChildId()`. The owning identifier is escaped inside the composite, so literal slashes and percent signs remain part of the parent identity. Route segments are still independently URL-encoded. See the [whole-prototype review](wireframes/2026-09-13-code-review.md) for regression and performance evidence.
