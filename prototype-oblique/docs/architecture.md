...
The application scripts load in the order above; each is an IIFE that adds one object to the `window.DK` namespace. `catalog-config.js` creates the namespace and `boot.js` follows it as the second script: the four UI files and the `read_snapshot()` request start as soon as those two small files and the stylesheets have arrived, while the other scripts are still downloading, and `data.js`/`catalog.js` take those responses over (`DK.boot.take()`) instead of requesting again. Both stay ordinary parser-blocking scripts at the top of the body: a synchronous script placed before the stylesheets in `<head>` stops the browser from preloading everything after it, and `async` would make the configuration's availability a race. The head preconnects to the Supabase origin and preloads the latin font. Data precedes its consumers, and `app.js` initializes last. Swagger UI is not in `index.html`: `api.js` inserts its stylesheet and bundle on the first visit of `#/api`. Likewise, `excel.js` loads the local ExcelJS bundle only on the first Excel export. `pdf.js` loads jsPDF, svg2pdf.js and matching static fonts when opening the first model export. Failed loads can be retried.
