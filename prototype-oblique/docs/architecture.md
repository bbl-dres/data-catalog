...
| File | Owns | Does not |
|---|---|---|
| `ui.js` | String helpers, safe link rendering, blob downloads, tiny widgets (including `tabPanel()`, which makes a panel a keyboard stop only when it holds no focusable content), table headers and locale-aware stable sorting, the i18n dictionary | Read app state |
...
| `views.js` | HTML for everything except the profile page body; `views.context()` derives titles, breadcrumbs, group options and actions from a route; `views.helpHost(state, { labelled })` renders the drawer's labelled help button and the side panel wraps navigation, heading and tree in `.ob-drawer-body` | Handle events |
...
| `app.js` | State, render coordination with focus restoration (including the tree, flyout and drawer scroll offsets), delegated events, export actions and handbook scroll tracking; decides the initial handbook scroll from the chapter requested before URL normalisation | Own handbook templates or vendor loading |

The print workspace owns a disposable event lifetime. Closing or rebuilding its language UI aborts listeners, disconnects observers and clears queued scroll work, including visual-viewport events and popover size observation. The compact-mode decision is shared by initial rendering and resizing; CSS consumes the app's visible-viewport tokens; `select-menu.js` and `diagram-export.js` read `--ob-menu-offset` for popover placement. Asynchronous asset/PDF completions check their owning session before publishing results. Print content is captured once, including documented product members and endpoint operations; paper layout and preview zoom remain independent.

`views.page(route, state)` returns the application markup as a string. `app.render()` replaces `<main>` while preserving focus and sidebar/flyout scroll offsets. Detail metadata uses three always-visible sections: Key facts, Protection and privacy, and System. `replaceHtml()` identifies focused controls by `data-focus`, `id` or data attributes; disappearing menu items map back to their trigger. Repeated table controls include their group instance in `data-focus`. Route changes skip this control restoration and normally scroll to the top; a plain `#/manual` also opens at the top, and only an explicitly requested chapter (`?ch=…`, including legacy IDs) scrolls under the header while the URL is still normalised to the canonical chapter.
...
Search ranks by `data.relevance()`: an exact name (100) beats a name prefix (90), a word prefix inside the name (80), any name substring (70), technical-name hits (50/40) and description hits (20/10). Suggestions keep compact type groups. Results use one table, ordered across all types before pagination by `search.page()`; the sort selector offers relevance, name and modification date. The description cell uses the shared summary clamp (two lines in tables, three in cards), as do a system's data-table rows. Matching tolerates umlaut spellings ("gebau", "gebaeu" both find "Gebäude") and hits are highlighted with `ui.highlight()`.
...
Field order and relative sizing are shared by web and print: name/description, context, ownership, type-specific facts, version, counts and status. Code-value tables retain code first. Counts are optional and enabled by default. Web tables reserve minimum readable widths (14 em for short enumerated columns, 18 em for descriptions with weight 3), then give spare space mainly to descriptions and names; the search result table uses the same em sizing instead of fixed percentages, so it switches to cards on the same basis. Sortable header buttons fill their cells and are 44 px targets on touch layouts. Their existing container observer switches to labeled cards when the selected columns cannot fit, including sidebar resizing and text scaling. Tiles and mobile table cards share label-width and summary-length tokens. PDF columns use the same width priorities with physical font/header measurements and complete text.
