...
│   ├── boot.js           DK.boot   – starts the UI-configuration and snapshot requests before the application scripts arrive
...
The application scripts are IIFEs on `window.DK`. `catalog-config.js` creates the namespace, followed by `resources.js` and `boot.js`. Those small scripts start the four UI files and the snapshot while later application scripts are still arriving. `DK.boot.take()` hands out each parsed-result promise once; failures are observed immediately and remain available to the normal load-error display. Shared requests have a 20-second deadline that covers body parsing.
...
The loader fetches local UI configuration alongside one Supabase `catalog.read_snapshot(true, false)` request; both start in `boot.js` before the application scripts. That snapshot carries every current collection but no change events. A profile's Verlauf tab reads its owner's history once through `catalog.read_history()` (`data.historyState()` reports loading, rows or failure and re-renders the page when the read completes) and keeps it until the next catalog load; attribute and field profiles show their owner's list without another read. A complete snapshot, as the fixture tests and older clients supply it, still projects its `change_event` collection directly. The UI files use the normal HTTP cache (the same `max-age` as scripts and styles), the snapshot request is never cached, and a failed early request surfaces as the loader's own error. The invoker-rights SQL functions read normalized records in one statement each; they store no JSON mirror and are not subject to collection row truncation. `catalog.js` projects records and direct references into the existing view shapes. Parent-scoped child identifiers preserve existing URLs. Catalog names/descriptions are resolved from stored language columns; display fallback never changes stored translations. Search, sorting and pagination remain in memory.
