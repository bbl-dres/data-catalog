...
The later `api_write()` command extends REST access to 16 resource types, including actor, quality rule, relationship and lineage records. It retains the same permanent-user, property, revision, transaction and audit boundaries; system-managed fields and audit history remain protected. DELETE archives; no catalog record is physically removed. The `catalog-api` Edge Function uses only the publishable key and caller token. Public GET routes require its gateway JWT check to be disabled, but every write is checked with Auth and then re-authorized by the Data API/SQL. `If-Match` and per-command idempotency keys are required. Account tokens are revealed/copied on request, cleared from the dialog on close and never include refresh tokens. Their expiry is shown. Both write RPCs now check the live Auth account and session before validation, locks or idempotency receipt lookup. Server-confirmed sign-out/revocation, deletion, an active ban, anonymous accounts and an expired session end time deny editing, including replayed commands. Local-only logout during an outage does not revoke a server session. See [REST setup and use](api.md).

The [session security migration](../supabase/migrations/20260913050000_catalog_session_security.sql) also removes additive future grants in the unused public schema and revokes the obsolete membership helper. It preserves the legacy membership rows for history. No editor allowlist is introduced: every active permanent app account with a live session can edit. The gateway still verifies JWT signatures, role and expiry. Auth-managed users/sessions remain private; the private helper exposes only a boolean and pins an empty search path. It checks account/session ownership and does not trust editable user metadata. This follows [Supabase’s revocation guidance](https://supabase.com/docs/guides/auth/sessions#how-to-ensure-an-access-token-jwt-cannot-be-used-after-a-user-signs-out). Session inactivity policy is still enforced by Auth; this guard does not invent a separate idle timeout.

The two exposed SECURITY DEFINER write RPCs are intentional, reviewed command boundaries: they need owner rights because direct writes are denied. Moving the same commands behind thin wrappers would not remove that trust boundary. Keep their grants and allowlists narrow. The four private RLS-without-policy advisor entries intentionally deny all row access; do not add permissive policies to silence them.
...
The 13 September hosted response has HSTS but no HTTP CSP, X-Frame-Options, nosniff or Referrer-Policy headers. The meta CSP and no-referrer policy remain active. As defense in depth, boot/Auth initialization and app mounting stop inside any frame; the page offers only a link to open the catalog separately, without forwarding callback query/fragment tokens. Browser tests cover a cross-origin parent and sandboxed frames. A hosting-level `frame-ancestors 'none'` policy remains preferable and must be configured on a host/proxy that supports headers; GitHub Pages does not supply it through repository files.

## Dependency verification

[vendor/manifest.json](../vendor/manifest.json) pins all five browser libraries and the Swagger stylesheet to SHA-256 file hashes and SHA-512 npm archive integrity values. Vendor Git attributes preserve those bytes across platform line-ending settings. Notices and licenses remain beside the assets. The verifier makes no repository changes:
...
The [test guide](../tests/README.md) covers setup. Run `session-security.cjs`, `security-sql.cjs`, `security-browser.cjs`, `api-contract.cjs` and `api.cjs` for the dedicated boundaries, plus affected regression suites. Security checks use isolated PostgreSQL and intercepted browser responses; they do not probe writes against the hosted project. Current findings, hosted verification and remaining operational items are recorded in the [13 September security review](review/2026-09-13-security-review.md); the [7 September review](review/2026-09-07-security-review.md) remains historical evidence.
