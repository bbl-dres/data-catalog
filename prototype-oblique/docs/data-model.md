...
- [ ] **Relationships:** the [ten relationship types](#relationship-types), verification, coverage and endpoint scope cover the associations needed at launch. Business-instance cardinalities and arbitrary new relationship types are not implied by these signatures.
...
| Hosted activation and recovery | Database activation and security verification are completed, including private grants, policies, triggers and ledger reconciliation. Browser save/history/restore passed. REST Edge deployment and backup/recovery operations remain separate from that completed review; see the [queue](todo.md). |
...
| Quality requirements | Reusable rules and both assignment junctions | 31 rules, 187 business-attribute assignments, no field assignments | Required flag only; assigned custom rule definitions are not displayed | Required shortcut in browser; complete rules/assignments via REST after activation |
...
| Relationships and lineage | Ten allowed relationship types; separate technical lineage | 119 relationship records: 117 candidates and 2 confirmed specializations; no lineage records | Field/attribute mappings show candidate/confirmed status and linked tables; table/object realization still selects the first match | No browser relation/lineage editor; REST after activation |
...
DataTable lists (including system profiles), attribute and field tables default explicitly to sortOrder ascending, including when the rank column is hidden. Column sorting is temporary view state. **Ansicht / View** offers the stored position as an optional **Zeilenreihenfolge / Row order** column on owned rows, hidden by default. Sorting this column ascending restores the saved sequence without editing data; its values stay fixed when filtering or sorting. For Attribute and Felder, Zeilenreihenfolge is first in both Ansicht and the table when enabled; their common fields share a logical sequence and Status stays last. Visibility defaults remain unchanged. The same optional column is available in print. There is no separate saved-order button. Exporting a profile follows its selected row sort, but its `sortOrder` cells retain the stored values. Entire-catalog exports use alphabetical top-level entries and saved child order. `position` in legacy projections is a calculated ordinal and must not substitute for `sortOrder` or source-file positions. An offline fixture without a saved rank exports an empty cell.

**Content ordering, 13 September 2026:** the original 217 business-attribute definitions across 26 populated objects now have unique spaced ranks. Primary identities lead; address proceeds country, region, city, postal code, street, house number, supplement. Topic bands begin at 100, 200 and so on; retired/archived definitions begin at 9000. These are authored presentation ranks, not structural PropertySet membership or enforcement of business keys. The [ordering review](review/2026-09-13-attribute-order.md) records the exact map and audited verification. The existing editor movement described above can compact ranks; the spacing convention does not alter that behavior.

**DataTable** also has a saved `sortOrder` within its system. Its default is 0, and browser/REST creation preserves that default unless an explicit rank is supplied; the child append rule above does not apply to tables. The profile editor exposes the number. Table lists use it by default; explicit user sorts remain respected. On 13 September 2026, all 30 tables and 621 fields received spaced ranks through guarded audited updates. Source names, key roles and definitions were preserved. [Plan and verification](review/2026-09-13-source-order.md). Other top-level entities have no manual order in this revision. Future property sets need their own group order; grouped attributes use their own `sortOrder` within each group. Group membership and group order remain proposed features, not implicit meanings of the current field.
...
**Content follow-up, 13 September 2026:** six new Gebäude definitions retain the spaced ordering convention. The building profile, four accuracy categories, seven standards, concise comments, BBL responsibility assignments and three candidate Baujahr mappings are [applied and verified](review/2026-09-13-catalog-refinement.md). No schema, grant or RLS changes were needed. The [classification follow-up](review/2026-09-13-building-classifications.md) adds two separate Gebäudeart definitions and two GWR list bindings; the combined Gebäudeart definition is archived. The [condition/heritage additions](review/2026-09-13-building-condition-heritage.md) add two draft text attributes and two candidate RE-FX links; NF/HNF and apartment counts are excluded.

**Latest applied content:** [Component classification and warranty](review/2026-09-13-component-attributes.md) adds six definitions to each of Bauteil, Technische Komponente and Technische Anlage. [document concepts and IDs](review/2026-09-13-document-concepts.md) records shared Dossier attributes, EFV responsibilities, Geometriebezug removal and the ID naming convention. The [seven basic architectural quantities](review/2026-09-13-basic-measurements-proposal.md) and [23 missing spatial FK attributes](review/2026-09-13-spatial-fks.md) are applied. Areal remains separate; only the existing FK labels are used, with no explicit-target metadata or schema change.

### Content readiness review

**Content decision, 13 September 2026:** geometry values use Geometrie; business lifecycle labels use Status, except Gebäude’s Status (GWR), bound to GSTAT. Bemessung has ten active attributes plus seven measuredFor target-type relationships; separate subject fields, Bemessungsumfang and Ermittlungsart are archived. FID is labelled FID (AOID). [Latest simplification](review/2026-09-13-simple-attributes.md). The [applied cleanup](review/2026-09-13-bemessung-simplification.md) changes catalog content only. A [subsequent standard-list addition](review/2026-09-13-bemessung-standard.md) binds Bemessung’s Standard to BBL Bemessungsstandard with five choices; edition and rule details stay in Quelle. It does not create operational measurement tables or change SQL columns.
...
|---|---|---|
| Business definitions | 42 BusinessObjects (38 active) and 295 BusinessAttributes, including archived definitions; the seven reviewed profiles contain 104 direct definitions plus ten visible referenced measurements. Twelve measurement definitions are visible across all profiles | Review the [current attribute and measurement profiles](business-object-attribute-proposal.md), key components, grouping and source evidence. Distinguish direct attributes from values reached through measurement concepts; approve each relevant definition or record its remaining gap. |
...
| Requirements | 31 rules: 2 required and 29 custom; 187 business-attribute assignments and 0 field assignments | Review each rule's applicability, scope and assignment, including retired rules. An empty assignment set is not proof of optionality or lack of constraints. Assigned rules need a usable review surface before users can approve them through the app. |
| Mappings and service support | 121 relationships: 2 confirmed specializes and 119 candidate relationships: 21 realizes, 66 exposes, 14 basedOn, 5 sourcedFrom, 1 servedBy, 7 measuredFor and 5 represents | Review the evidence for each required mapping. The three Baujahr and two condition/heritage represents mappings are candidates. There are no correspondsTo or assesses records and no lineage records. Empty sets mean undocumented/unassessed coverage, not proven absence of a relationship or service capability. |
| Vocabularies and product definitions | 95 code lists, 2,120 code values; 5 products and 12 product attributes | Check code-list applicability, editions, descriptions and product contracts. No code-value parent or product-attribute business binding is populated; decide where those links are actually required. |
| Lifecycle, language and responsibility | 12 attributes, 2 code lists, 1 product and 1 requirement have status retired; explicit contact-actor assignments are empty | Review retirement and continued reference use, required language coverage and the source of inherited roles. Empty managed contacts may be valid when the responsible organisation's contact page suffices. |
...
| `rowVersion` | Edit revision | Bearbeitungsrevision | — | Integer | 1 | Automatically maintained edit revision; initially 1 and advanced on stored changes, including owned edits. Separate from catalog definition version. |
| `editedAt` | Last edit timestamp | Zeitpunkt der letzten Bearbeitung | — | Timestamp | 0..1 | Server time of the latest app/REST edit; unknown for earlier imports. Separate from source freshness, definition version and historical dates. |
| `sortOrder` | Row order | Zeilenreihenfolge | — | Integer | 1 | Display order within the system, from 0 through 2147483647 (SQL integer); defaults to 0. Reordering preserves identity. Ties are permitted. |
| `isArchived` | Archived | Archiviert | — | Boolean | 1 | Defaults to false. Hides the entry from normal browsing while retaining identity, references and history; independent of status/verification. |
| `createdOn` | Created | Erstellt am | — | Date | 0..1 | Date the catalog record was created; unknown historical dates remain unknown. |
| `modifiedOn` | Last modified | Zuletzt geändert am | — | Date | 0..1 | Date the catalog record last changed; not before createdOn. History and edit revision establish order. |
| `name_de` | Name (DE) | Name (DE) | — | Text | 0..1 | German name; at least one language is required. Not an identifier. |
...
| `specializes` | BusinessObject | BusinessObject | Source is a specialization of the target business concept. Active assertions must form an acyclic graph; confirmed specializations display the parent’s shared attributes with links to their defining object. No duplicate rows, implicit overrides, role or responsibility inheritance. |
...
The association types basedOn, sourcedFrom, servedBy, measuredFor and specializes omit endpoint scope, coverage, supportStatus and assessedServiceVersion. Optional rule notes explain scope; comment is a publicly readable catalog note. Update the single scoped record with ChangeEvents.
...
| [DataTable](#datatable) | `data_table` | 34 |
