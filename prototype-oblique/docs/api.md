...
The API page at #/api documents public reads and authenticated CRUD. Swagger UI loads the generated [OpenAPI 3.1 contract](../data/swagger.json). Collection reads and the versioned read_catalog_index / read_record RPCs use Supabase PostgREST; standard POST, single-record GET, PATCH and DELETE use the catalog-api Edge Function.
...
| Read parent records and counts | `POST /rest/v1/rpc/read_catalog_index` with `{}` | `Content-Profile: catalog` |
| Read one owner and its children | `POST /rest/v1/rpc/read_record` with `record_table` and `record_id` | `Content-Profile: catalog` |
...
The four public read RPCs use SQL `STABLE`, security-invoker functions. They read metadata and do not write records. There are 20 public table reads, including the read-only version singleton; the underlying business data remains outside the catalog API.
...
The app starts with POST /rest/v1/rpc/read_catalog_index and body {}. It receives full parent records, structural relationships without evidence, endpoints, quality rules, childCounts keyed by owner UUID, and catalogVersion as decimal text. POST /rest/v1/rpc/read_record with {"record_table":"data_table","record_id":"<uuid>"} returns the owner, its complete owned inventory (including archived rows), inherited attributes, full relationship evidence, quality assignments and minimal counterpart children. inheritedAttributes maps attribute UUIDs to defining object UUIDs. Counterparts carry _counterpart:true and must not be mistaken for complete editable records. Merge a bundle with an index at the same version. Both RPCs accept if_version; an unchanged revision returns only schemaVersion, catalogVersion and notModified:true. Unknown owners fail, and the owner table is allowlisted.
...
The legacy snapshot contract remains available: `read_snapshot(true,false)` includes independent API fields and omits history; `read_snapshot()` keeps the original table-field-only snapshot with history. The app requests the full snapshot only for catalog Excel export or when the new index RPC is unavailable during rollout. Quality thresholds are exact decimal strings. `read_history(record_table,record_id,max_events)` still returns an array of owner and owned-child events, newest first, without before/after states. Its default limit is 1000, capped at 5000. These legacy response shapes intentionally retain compatibility and do not gain a version envelope.
