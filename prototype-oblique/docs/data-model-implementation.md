...
| Entity | SQL / REST resource | Current browser coverage |
|---|---|---|
| [Actor](data-model.md#actor) | `actor`, CRUD | Existing actors in responsibility selectors; no actor-management page. |
| [BusinessAttribute](data-model.md#businessattribute) | `business_attribute`, CRUD | Owner rows and attribute profiles; Required shortcut, selected descriptive fields. |
| [BusinessObject](data-model.md#businessobject) | `business_object`, CRUD | Collection, profile and owned-attribute editing. |
...
| [ProductAttribute](data-model.md#productattribute) | `product_attribute`, CRUD | Rows in the owning product; selected descriptive properties. |
| [QualityRequirement](data-model.md#qualityrequirement) | `quality_requirement`, CRUD | Required shortcut uses shared rules; no general rule registry editor. |
| [Relationship](data-model.md#relationship) | `relationship`, CRUD | Derived relation groups; relations stay read-only in edit mode. |
| [System](data-model.md#system) | `system`, CRUD | Collection/profile editing and derived structure lists. |

ServiceEndpoint is stored in `service_endpoint` with its own UUID, revision, order and archive flag, while remaining owned by DataService. The two quality junctions are read-only tables; REST changes their complete ID collections through the owning attribute/field. This gives 19 public tables and 16 mutable REST resources without changing the 16-core-entity model.
...
Concrete entity tables use native relational constraints. Bounded owned values use validated JSONB; conceptual RecordReference shorthand is stored/exposed as typed UUID foreign-key columns.
...
| LineageRelation | `lineage_relation`; directed technical endpoint FKs and one relation identity per pair, including inactive records. |
...
Every core entity table has a stable external `identifier`, unique within that table, including full child identifiers. ServiceEndpoint identifiers are scoped to their owning DataService; junction tables need only their endpoint keys. Internal UUIDs are used by REST record routes/FKs; browser routes use readable identifiers and owner context. Legacy reused child names were mapped to distinct full identifiers; never merge fields because both are named EGID. Mutable core records and endpoints have a positive row_version, initially 1. App/REST commands check expected revisions; administrator SQL/import scripts must perform their own checks. ChangeEvents are append-only and have no edit revision. Documentation-link and owned-row changes advance the applicable owner revision.

Singular relationships use concrete FKs, for example `data_field.data_table_id → data_table.id`. Applicable data-owner, steward and contact ID columns reference `actor.id`; custodian FKs exist only on the four technical entity tables; inline organisation values use JSONB. Explicit product associations use the Relationship table, with the DataProduct as source; the three legacy product arrays have no additional authoritative junction tables. Quality assignment junctions use `(business_attribute_id, quality_requirement_id)` or `(data_field_id, quality_requirement_id)` as the composite PK; both members are FKs and no separate assignment ID is needed. The API exposes UUID arrays for quality assignments; authoritative relationship IDs do not live only in JSONB.

### Resolving assigned requirements

Each owner uses its quality junction as the only assignment store. Join to QualityRequirement to obtain its names, descriptions, rule type, threshold, version and catalog status. No copied rule text or per-assignment overrides are needed. Bind the owner UUID from the API as $1 below; field queries use the data_field_quality_requirement junction. Apply the normal language fallback when displaying the joined definitions.

```sql
SELECT rule.*
FROM catalog.business_attribute_quality_requirement AS assignment
JOIN catalog.quality_requirement AS rule
...
The owned-row representation has `id UUID PK` and `data_service_id UUID FK` to DataService, plus row_version, created_on, modified_on, edited_at, sort_order and is_archived. These are public REST fields; the browser hides technical identities. The composite service/endpoint key prevents references crossing owners.

Endpoint edits match UUIDs within their immutable service owner. Reordering preserves identity. Browser omission from a patch leaves the endpoint unchanged; explicit row removal and REST DELETE archive it. Referenced and unreferenced endpoints are retained, with no physical deletion or automatic assertion-status cascade. At least one of URL, relative path or operation name is known. Supporting links belong to the DataService; their title or the review summary identifies the operation. Record the scope/result of endpoint checks in the service ChangeEvent summary. No per-endpoint source payload is required.
...
RecordReference is conceptual shorthand, not an alternative REST payload or a polymorphic database FK. Keep the small supported target set explicit:
...
| ChangeEvent before/after | Historical JSONB snapshots, never a second writable source of current metadata. Current snapshots preserve SQL names and UUID FKs; they are publicly readable. Exact authenticated-user attribution remains private. |
...
Import tools must reject duplicate JSON keys before JSONB conversion and hash the original capture. The current REST handler uses JSON.parse and does not reject duplicate object keys separately; clients must send unique keys. SQL NULL represents unknown optional scalars; regular collections use empty arrays. The keyRoles exception is deliberate. Omit unknown translation keys inside owned JSONB and reject blank translations or explicit null stored as a translation value. REST replaces a supplied owned JSON value as a whole; remove its optional key in that replacement to clear it.
...
Index FK/owner lookups and actual locale-aware search paths. Add JSONB indexes only for demonstrated queries. Public reads use the shipped publishable key. Authenticated writes use the current app access token and audited commands; database passwords and elevated keys never belong in the browser or catalog content. [Editing rules](#editing-review-and-imports) define atomic audit writes and [migration](#migration-from-the-current-prototype) defines cutover and recovery.
...
Illustrative conceptual record with all four supported name/description columns populated. This uses dictionary names, not a REST request:

```json
{
  "id": "223a23d4-9e26-48ea-8a60-4ce0b74d5300",
  "identifier": "energy",
  "kind": "domain",
  "rowVersion": 1,
  "isArchived": false,
...
The example omits optional unknown values and empty collections. It defines catalog metadata, not operational energy readings. The running app reads SQL-backed snapshots; REST uses row_version/is_archived and omits derived kind. See the generated API create schema for actual writable input fields.
...
The [browser edit mode](edit-mode-implementation.md) edits profiles and owned rows through `catalog.save_entry()`. The [REST API](api.md) adds `catalog.api_write()` for all mutable resource types, including actors, quality rules, relationships and lineage. Both require a permanent authenticated app user, compare revisions and save history atomically; direct table writes remain denied. The browser still excludes relationship/lineage, actor and general rule management, terminology edits and verification assessments. Hosted activation is documented separately.
...
The app and REST commands authenticate a permanent app user and use frozen table/property inventories. SQL locks/checks current revisions, applies constraints, and commits changes, events, private attribution and an idempotency receipt in one transaction. A retry of the same command returns its original result; reusing its key with different input or another user is rejected. Direct administrator SQL still needs an explicit audit/import script.

| Operation | Current behavior |
|---|---|
| Scalar or owned JSON value | Patch supplied fields and validate the resulting record. A no-op produces no revision/event. REST replaces each supplied JSON object/array; it does not deep-merge it. |
| Quality assignments through REST | Replace the complete quality_requirement_ids collection on the attribute/field. Validate referenced rules and compatibility; revise/audit the owner record with old/new IDs. |
| Browser Required shortcut | Link/unlink active required rules through the helper. Audit records the effective boolean; exact assignment-ID history is a known limitation. |
| Owned child or endpoint | Validate the child revision and immutable owner. Advance the owner revision as well; a revision-only owner touch does not require another event. Browser profile commands can include several explicit owned edits atomically. |
| Relationship or lineage | Edit the assertion with its own revision. No implicit product or endpoint revision change; related records are not rewritten. Changing endpoint/type identity requires a distinct assertion. |
| Shared quality rule | Edit/audit the rule and validate explicitly incompatible business uses. Existing assignments read the current definition; their status/revision does not cascade. |
| Hierarchy | Validate paths and owner scopes under the shared write lock so concurrent writes cannot create cycles. |
| Status or verification | Validate the selected token, required content and applicable scope/version transitions. Evidence quality remains an editorial responsibility. |
| Archive / restore | Change is_archived and preserve identities, references and previous lifecycle/verification. No hard delete or cascade. A separate rejected/obsolete-to-active transition must first return to candidate. |

The write guards serialize cross-record changes with the existing advisory lock. Use READ COMMITTED or SERIALIZABLE; REPEATABLE READ writes are rejected. A stale caller revision must never be silently replaced by a fresh one. See [API headers and retry rules](api.md#crud-routes-and-tokens).

### History attribution

Current commands emit generic public Catalog editor labels and store the authenticated user UUID and command ID in catalog_private.edit_event_actor. They do not set a contact Actor or expose a login email. Previously recorded actorName/actorId values remain immutable even when that Actor is edited. ChangeEvent and its snapshots are publicly readable; private exact-user attribution and receipts are not. Catalog sensitivity labels do not alter this access policy.

### Audit snapshots and event grouping

New app/REST records receive catalog creation/modification dates and an edit timestamp from the server. Unknown historical creation dates remain unknown. Event timestamps identify real edits; they do not backfill import or source freshness. Use the returned row_version for concurrency; owned edits may advance an owner more than once within a command.

Current snapshots use SQL snake_case properties and UUID FKs. REST attribute/field events contain complete quality_requirement_ids. Browser Required events contain the derived required boolean instead; the helper may create its reusable rule without a separate rule-creation event. Full reconstruction of those assignment IDs from browser events is therefore not supported yet.

Endpoint events target their DataService but carry the changed endpoint's ID and before/after row. Other child events target the child; the UI can group them under its owner. Snapshots do not expand every sibling, referenced actor, source payload or derived view. Changes made together share the command's import_id/private attribution. Restoration creates a new event; history is never rewritten.

A fuller aggregate audit, with complete owned collections and a standalone event for every implicitly created shared rule, remains a follow-up. It must not be assumed from the current generic history display. Arbitrary spreadsheet batches and source-refresh merges below are also future tooling; REST currently writes one resource per request, with owned quality assignments atomic within it.

### Batch editing contract

These are requirements for future general batch/import tooling. Existing prepared SQL update scripts have their own guards; the REST API does not provide a bulk-import endpoint.
...
The browser's catalog adapter resolves snapshot UUID references into existing kind/identifier view models; REST keeps SQL UUID columns. Translate legacy route/JSON kinds using the adapter table below; a target kind is not the plural of its class name. Retain the current global search scope: BusinessObject, DataTable, CodeList, DataProduct, DataService, Domain and System. Attributes/fields keep their local searchable tables. A future server-side UNION ALL projection could serve the global result list without a new writable entity table.
...
The current SQL-backed renderer uses persisted represents assertions for authoritative attribute/field correspondence. Any name-based suggestion in an offline fixture is not an authoritative link. New mappings require identity resolution and review. Confirmed mappings, API gap reports and lineage must not inherit certainty from a visual connection or a similar label.
...
This section preserves the historical JSON-to-SQL migration specification. The initial cutover is complete; do not reimport frozen fixtures over curated live records. Current activation steps are in the [SQL setup](../supabase/README.md) and [API guide](api.md#activation). The detailed property-by-property conversion is in the [appendix](#appendix-json-property-migration).
...
| `data/swagger.json` | Generated public-read/authenticated-CRUD contract for the implemented Supabase schema; regeneration and validation are in the [API guide](api.md). |
...
The table records differences in the frozen pre-SQL fixture renderer, not current SQL-backed behavior. Current coverage is described [above](#prototype-coverage).
...
Historical sequence and remaining targets: normalized storage, identities, import, public reads, quality/lineage tables and edit/REST commands now exist in the repository. Hosted activation, general import workflows and later visualization/publication requirements must be assessed separately.
...
These criteria combine implemented checks with later workflow/read-model targets. Local SQL and browser evidence is described in the [test guide](../tests/README.md), [editing guide](edit-mode-implementation.md) and [API verification](api.md#verification); this table does not claim every future workflow has passed or that hosted activation is complete.

### PostgreSQL implementation acceptance cases

Use the current SQL/REST/browser suites for implemented behavior. Rows involving general source-refresh merging, aggregate audit, complete multi-domain projection or backup recovery remain acceptance requirements for that future work.
...
| Exact numbers | Decimal `"0"` survives API → SQL → API; high precision is unchanged. Unsafe Integer values, non-string Decimal values inside owned JSONB, NaN and infinity fail. Scalar REST comparison_value also accepts finite numbers; use strings for precision. |
...
| Audit aggregate | REST quality changes retain old/new IDs; endpoint/child edits advance the owner revision. Browser Required snapshots and complete aggregate history have the limitations documented above. Assertion edits do not implicitly revise products. |
| Manual status and audit grouping | Status changes are ordinary edits with no review-date/reviewer columns. Related command events share importId; revision-only owner touches do not require another event. Unrelated records keep their status/revision. |
...
| Relationship uniqueness and scope | Duplicating a canonical type/source/target/endpoint scope fails, including absent scope. Invalid endpoint kinds, self-links and forbidden assessment fields fail. Inverse display creates no row. Reject stale assertion revisions; no separate product revision is currently required. |
...
| Endpoint collection replacement | Reordering preserves endpoint identities. Explicit removal archives the endpoint and preserves its references/history; omitted rows remain unchanged. Physical deletion is denied. A null endpoint is valid only for the permitted relationship signatures. |
...
| Quality and lineage | QualityRequirement rules are reused through attribute/field reference collections; LineageRelation is separate from semantic Relationship and does not imply execution history. |
...
Document consistency checks verify the specification and its mapping. Executable SQL/browser suites verify implemented behavior; neither proves hosted activation or factual completeness of catalog content.
...
- [PostgreSQL JSON types](https://www.postgresql.org/docs/18/datatype-json.html), [constraints](https://www.postgresql.org/docs/18/ddl-constraints.html) and [date/time types](https://www.postgresql.org/docs/18/datatype-datetime.html) — reference documentation for persistence capabilities. These links do not assert the hosted project's PostgreSQL version; check that separately during deployment.
