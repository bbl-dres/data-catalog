...
def inspect_model(path):
    if path.suffix.lower() == '.png':
        model_path = ROOT / 'docs/sources/sap-refx/sap-refx-model.json'
        matches_path = ROOT / 'docs/sources/sap-refx/sap-refx-field-matches.json'
...
def apply_table_context(tables, decisions, captured):
    ...
    path = ROOT / 'docs/sources/sap-refx/sap-refx-definitions.json'
...
def import_data_sources(tables, selections, curation_hash, captured):
    ...
    path = ROOT / 'docs/sources/sap-refx/sap-refx-data-sources.json'
    snapshot = json.loads(path.read_text(encoding='utf-8'))
...
def curate_catalog(tables, model, structures, provenance, captured):
    ...
    path = ROOT / 'docs/sources/sap-refx/sap-refx-catalog-curation.json'
...
def main():
    ...
    related_updates = []
    if has_diagram:
        report_path = ROOT / 'docs/sources/sap-refx/sap-refx-import-report.json'
        ...
        api['sourceReconciliation'] = {'scope': 'source-diagram-only', **reconciliation['summary'],
                                       'report': 'docs/sources/sap-refx/sap-refx-import-report.json',
    ...
    if existing:
        ...
        apis.append(api)
    write_json(ROOT / 'docs/sources/sap-refx/sap-refx-import-report.json', report)
