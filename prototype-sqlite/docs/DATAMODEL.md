...
| `key_role` | `TEXT` | YES | `PK`, `FK`, `UK`, or NULL. Marks the attribute's role in the business key structure of its concept. Used for exports and diagram rendering. |
...
| `status` | `TEXT` | NO | `adms:status` | `draft \| approved \| deprecated` — lifecycle state |
| `owner_id` | `UUID` | YES | `dcterms:creator` | FK → `contact.id` — the steward/owner of this list |
...
| `min_value` | `TEXT` | YES | — | |
| `max_value` | `TEXT` | YES | — | |
| `completeness_score` | `FLOAT` | YES | `dqv:value` | Vollständigkeit — composite completeness 0.0–1.0 |
| `format_validity_score` | `FLOAT` | YES | `dqv:value` | Formatkonformität — share matching expected format |
| `timeliness_score` | `FLOAT` | YES | `dqv:value` | Aktualität — age relative to expected refresh cycle 0.0–1.0 |
| `accuracy_score` | `FLOAT` | YES | `dqv:value` | Genauigkeit — agreement with authoritative sources 0.0–1.0 |
| `consistency_score` | `FLOAT` | YES | `dqv:value` | Konsistenz — cross-system coherence 0.0–1.0 |
| `uniqueness_score` | `FLOAT` | YES | `dqv:value` | Eindeutigkeit — share of rows without unintended duplicates 0.0–1.0 |
