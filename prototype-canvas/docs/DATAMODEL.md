...
| Discovery & navigation | Controlled tag vocabulary with i18n | Promote today's free-text `node.tags TEXT[]` to a curated `tag (id, slug, label_de/fr/it/en, description_de/fr/it/en, parent_tag_id, deprecated)` table, with a `node_tag (node_id, tag_id)` join replacing the array column. Optional `parent_tag_id` allows shallow hierarchies (`personendaten → personendaten/besonders_schutzenswert`, `geo → geo/koordinaten`). Stops the drift inevitable in free-text arrays, brings translations into the data model itself rather than relying on the application's i18n.json `tag.*` namespace (which today decouples tag labels from the catalog and is easy to forget at insert time), and lets the Views feature filter on stable tag ids instead of strings. |
...
| Catalog richness | External identifier registry | A node may be known to the outside world by several identifiers in different schemes: TERMDAT term id, eCH register id, opendata.swiss URI, SAP catalog code, Fedlex SR-number, Geocat UUID, Wikidata Q-id. New `node_external_id (id, node_id, scheme, value, url, valid_from, valid_to)` with `UNIQUE (scheme, value)` per organisation and a `scheme` registry seeded with the schemes BBL actually uses. Distinct from `standard_reference` (which models the standard itself, not an entry inside it) and from `tags` (free-text keywords without resolvability). Powers cross-catalog deep links, federated harvesting, and DCAT-AP CH `dct:identifier` projection at export time. |
| Catalog richness | Column-level lineage with transformations | Today `flows_into` and `derives_from` only connect distributions, and the only attribute-level edge (`fk_references`) records structural foreign keys, not data movement. Lift both lineage edge types to attribute → attribute granularity and add an optional `edge_transformation (edge_id, language CHECK IN ('sql','python','dbt','manual','formula'), expression, notes_de/fr/it/en, source_ref)` side table that records the mapping logic. Answers impact-analysis questions distribution-level lineage cannot — "if SAP renames `BUKRS`, which downstream attributes break?" — and gives DCAT-AP CH and `prototype-sqlite` a more honest export of how data actually flows. |
